<template>
  <div>
    ...
    <nav-bread>
      <span slot="content">My Cart</span>
    </nav-bread>
              ...
              <li v-for="item in cartList" :key="item.productId">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a @click="cartEdit('checked', item)" v-bind:class="{'check': item.checked === '1'}" href="javascript:void(0)" class="checkbox-btn item-check-btn">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a @click="cartEdit('minus', item)" class="input-sub">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a @click="cartEdit('plus', item)" class="input-add">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a @click="cartDelConfirm(item.productId, item.productName)" href="javascript:void(0)" class="item-edit-btn">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a @click="toggleCheckedAll()" href="javascript:void(0)">
                    <span v-bind:class="{'check': checkedAllFlag === '1'}" class="checkbox-btn item-check-btn">
                        <svg class="icon icon-ok">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a @click="checkOut()" v-bind:class="{'btn--dis':checkedCount === 0}" class="btn btn--red">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal v-bind:mdFlag="mdFlag" v-on:close="closeModal()">
      <p slot="msg">确定删除 {{delProductName}} 吗？</p>
      <div class="btn--center"  slot="btnGroup">
        <a @click="cartDel()" href="javascript:void(0)" class="btn btn--m">确定</a>
        <a @click="mdFlag = false" href="javascript:void(0)" class="btn btn--m">取消</a>
      </div>
    </modal>
...
export default {
  ...
  methods: {
    ...
    cartDelConfirm(productId, productName) {
      this.mdFlag = true;
      this.delProductName = productName;
      this.delProductId = productId;
    },
    cartDel() {
      axios
        .post("/users/cartDel", {
          delProductId: this.delProductId
        })
        .then(res => {
          let data = res.data;
          if (data.status === "0") {
    ...
    toggleCheckedAll() {
      let checkedAll = this.checkedAllFlag === "1" ? "0" : "1";
      this.cartList.forEach(item => {
        item.checked = checkedAll;
      });
      axios
        .post("/users/checkedAll", {
          checkedAll
        })
        .then(res => {
          // let data = res.data
